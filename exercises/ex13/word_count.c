...
void new_free_pair(gpointer pair_data){
  Pair *pair = (Pair*) pair_data;
  g_free(pair);
}

void freeKV(gpointer key, gpointer value, gpointer user_data)
{
    free(value);
    free(key);
    free(user_data);
